...
async function handleImageUploadComplete(context: { request: Request, env: Env }): Promise<Response> {
  ...
  const chunkKeys = Array.from({ length: session.totalChunks }, (_, index) => `media-image-uploads/tmp/${uploadId}/${index}`)
  ...
  await bucket.put(finalKey, finalBlob, { httpMetadata: { contentType: session.mimeType || 'image/jpeg' } })
